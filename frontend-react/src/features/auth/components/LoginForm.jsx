  ...
  return (
    <>
      ...
      <div style={{ position: 'fixed', top: '50%', left: '50%', transform: 'translate(-50%, -50%)', width: '100%', maxWidth: '440px', background: 'var(--color-card)', borderRadius: 'var(--radius-md)', padding: '40px', zIndex: 1001, boxShadow: 'var(--shadow-modal)', border: '1px solid var(--color-border)' }}>
        <button onClick={onClose} aria-label="Đóng đăng nhập" style={{ position: 'absolute', top: '24px', right: '24px', background: 'none', border: 'none', cursor: 'pointer', color: 'var(--color-muted)' }}>
          <XIcon size={24} />
        </button>

        <h2 style={{ fontSize: '1.75rem', marginBottom: '8px', fontFamily: 'var(--font-display)', fontWeight: 300 }}>Chào mừng trở lại</h2>
        <p className="muted" style={{ marginBottom: '32px', fontSize: '0.95rem' }}>Nhập thông tin để đăng nhập tài khoản Furniq.</p>
        ...
        <p style={{ textAlign: 'center', marginTop: '24px', fontSize: '0.9rem', color: 'var(--color-muted)' }}>
          Chưa có tài khoản? <button className="btn-ghost" style={{ border: 'none', background: 'transparent', color: 'var(--color-text)', fontWeight: 500, textDecoration: 'underline', cursor: 'pointer' }} onClick={onSwitch}>Tạo tài khoản</button>
        </p>
      </div>
    </>
  );
